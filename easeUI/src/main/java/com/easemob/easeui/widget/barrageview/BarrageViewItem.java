 ...
 * 每一项的弹幕
 */
public class BarrageViewItem implements IBarrageViewItem {
...
//        strokePaint.setTypeface(Typeface.DEFAULT_BOLD);
     ...
     */
    public BarrageViewItem(Context context, CharSequence content, int startX) {
        this(context, new SpannableString(content), startX, 0, 0, 0, 1f);
    }

    public BarrageViewItem(Context context, CharSequence content, int startX, int startY) {
        this(context, new SpannableString(content), startX, startY, 0, 0, 1f);
    }


    public BarrageViewItem(Context context, SpannableString content, int startX, int startY,
...
//        }
    ...
    public boolean willHit(IBarrageViewItem runningItem) {
        ...
        float t1 = len1 / (runningItem.getSpeedFactor() * BarrageViewItem.sBaseSpeed);
        float len2 = t1 * mFactor * BarrageViewItem.sBaseSpeed;
        if (len2 > len1) {
            return true;
        } else {
            return false;
    ...
    public static void setBaseSpeed(int baseSpeed) {
        BarrageViewItem.sBaseSpeed = baseSpeed;
    }
