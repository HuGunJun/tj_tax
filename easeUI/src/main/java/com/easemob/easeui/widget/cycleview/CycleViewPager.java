...
import com.easemob.easeui.widget.viewpageranimation.TransformerForViewPager_two;
...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    ...
    private TextView tv_indicator;// 轮播文字
    ...
    private int ChooseID, NotChoseId;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(
                R.layout.ease_view_cycle_viewpager_contet, null);
    ...
    public void SetIndicatorResouse(int chooseID, int notChoseId) {
        ChooseID = chooseID;
        NotChoseId = notChoseId;
     ...
     * @param listener
     */
    public void setData(List<ImageView> views, List<CycleVpEntity> list,
    ...
    public void onPageSelected(int arg0) {
        ...
        tv_indicator.setText(infos.get(position).getTitle());
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setBackgroundResource(ChooseID);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition]
                    .setBackgroundResource(NotChoseId);
    }
