...
public class BarrageView extends View {
    ...
    private HashMap<Integer, ArrayList<IBarrageViewItem>> mChannelMap;
    private final java.util.Deque<IBarrageViewItem> mWaitingItems = new LinkedList<>();
    ...
    private void initChannelMap() {
        ...
        for (int i = 0; i < mMaxRow; i++) {
            ArrayList<IBarrageViewItem> runningRow = new ArrayList<IBarrageViewItem>(mMaxRunningPerRow);
        ...
        if (status == STATUS_RUNNING) {
            try {
                ...
                for (int i = 0; i < mChannelMap.size(); i++) {
                    ArrayList<IBarrageViewItem> list = mChannelMap.get(i);
                    for (Iterator<IBarrageViewItem> it = list.iterator(); it.hasNext(); ) {
                        IBarrageViewItem item = it.next();
                        if (item.isOut()) {
                            it.remove();
                        } else {
                            item.doDraw(canvas);
...
//                    Log.d(TAG, "start pick new item..");
                    IBarrageViewItem di = mWaitingItems.pollFirst();
                    if (di != null) {
     ...
     */
    private int findVacant(IBarrageViewItem item) {
        try {//fix NPT exception
            ...
            for (int i = 0; i < mMaxRow; i++) {
                ArrayList<IBarrageViewItem> list = mChannelMap.get((i + ind) % mMaxRow);
                if (list.size() > mMaxRunningPerRow) {//每个弹道最多mMaxRunning个弹幕
                    continue;
                }
                IBarrageViewItem di = list.get(list.size() - 1);
                if (!item.willHit(di)) {
    ...
    public void addItem(IBarrageViewItem item) {
        synchronized (mWaitingItems) {
            this.mWaitingItems.add(item);
        }
    }

    public void addItemToHead(IBarrageViewItem item) {
        synchronized (mWaitingItems) {
     ...
     * 是否新建后台线程来执行添加任务
     */
    public void addItem(final List<IBarrageViewItem> list, boolean backgroundLoad) {
        if (backgroundLoad) {
            new Thread() {
                @Override
                public void run() {
